...
- **허용(사용자 확인 없이)**: 로드맵(`docs/dev-log.md` "다음 작업")의 **결정이 끝난 다음 step 1개**를 구현 → `./gradlew test` 통과 → `feature/*` 커밋(Conventional Commits) → **dev로 `--no-ff` 병합·push** → dev-log 기록. (평소 워크플로 그대로, dev 병합까지 위임.)
- **금지(반드시 멈춰 사용자 몫)**: `main` 병합/푸시, 되돌리기 어렵거나 파괴적인 작업(운영 DB 데이터 삭제·위험한 스키마 변경), 외부 전송/공개, **새로운 의미 있는 결정**(버전·구조·범위·UX·스택). 이런 지점은 진행을 멈추고 dev-log "다음 작업"에 **`결정 필요: …`** 로 남긴다.
- **범위·안전**: 1 위임 = **1 step**. 테스트 실패·막힘은 되돌리지 말고 멈춰 보고. 끝나면 한 일 + 다음 결정 포인트를 요약. 로컬 Docker/MySQL이 없는 환경이면 단위 테스트(H2)까지만, MySQL 런타임 검증은 사용자 복귀 후.
