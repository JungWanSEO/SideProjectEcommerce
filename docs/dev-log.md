...
- **쿠폰/프로모션 진행(06-15~)**: Phase 2 후속 차별화로 착수. **Step 1 ✅(코어+체크아웃 적용)** = `coupon` 도메인(정액/정률·플랫폼/셀러 분담·플랫폼와이드/셀러한정)·체크아웃 코드 적용·gross 보존+payable·Flyway V25·**274 tests**·정적+MySQL 런타임 PASS. 작업트리=dev(미커밋). **다음 = Step 2(정산 분담: SettlementEntry discountAmount+fundedBy·net·대사·orders 분담 스냅샷 V26)** → Step 3 회원 쿠폰함 → Step 4 FE.
