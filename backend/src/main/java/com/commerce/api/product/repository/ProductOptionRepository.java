...
public interface ProductOptionRepository extends JpaRepository<ProductOption, Long> {
    ...
    @Modifying
    @Query("update ProductOption o set o.reserved = o.reserved + :qty "
            + "where o.id = :optionId and o.stock - o.reserved >= :qty")
    int reserve(@Param("optionId") Long optionId, @Param("qty") int qty);

    /** 소진(결제 확정): 예약을 실재고 차감으로 전환(stock↓·reserved↓). 예약이 보장돼 정상 1. */
    @Modifying
    ...
    @Modifying
    @Query("update ProductOption o set o.stock = o.stock + :qty where o.id = :optionId")
