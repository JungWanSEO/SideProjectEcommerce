...
상품 60종(세일·품절·재고임박·판매중지 섞임)·브랜드/셀러·리뷰·찜·쿠폰 4종(정액/정률/셀러부담/선착순)·멀티셀러 주문까지
채워지고, 재기동해도 중복되지 않습니다. **돈 흐름도 실제 경로로 시드**합니다 — 장바구니→체크아웃→결제(다중 PG)를
그대로 태워 셀러별 정산·지급 묶음·PG 대사(정산 후 환불로 인한 상태 불일치 포함)·반품 환불·역분개가 화면에 실제 데이터로 보입니다. 부하 테스트·스모크 테스트가 남긴 잔재(대량 더미 회원·쿠폰·빈 카테고리)도
같은 시드가 정리합니다 — 단, 주문·리뷰·감사 이력이 있는 데이터는 건드리지 않습니다.
