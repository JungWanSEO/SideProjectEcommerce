...
import { Category, CategoryCreateInput } from "@/lib/types";

/**
 * 카테고리 관리 화면 (/admin/categories, ADMIN).
 * 2단계 계층(최상위 → 자식)을 만들고 부모별로 그룹지어 보여준다.
 * 수정/삭제 API는 아직 없어 등록/조회만 — 후속(결정 필요: 자식 있는 부모 삭제·상품 참조 규칙).
 */
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          최상위 카테고리와 그 하위(2단계)를 만듭니다. 부모를 비우면 최상위로 등록됩니다. 수정/삭제는 후속 예정.
        </p>
      </div>
              ...
              <li key={root.id} className="px-4 py-3">
                ...
                {children.length > 0 && (
                  <ul className="mt-2 space-y-1 border-l border-gray-200 pl-4">
                    {children.map((child) => (
                      <li key={child.id} className="flex items-center gap-2 text-sm text-gray-600">
